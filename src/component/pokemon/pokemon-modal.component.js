...
export default class PokemonModal extends Component {
    ...
    async componentDidMount() {
        const pokeID = this.props.pokemon.url.split("/")[
            this.props.pokemon.url.split("/").length - 2
        ];
        const { pokemonIndex } = pokeID;

        // API to get pokemon details
        const pokemonUrl = `https://pokeapi.co/api/v2/pokemon/${pokeID}/`;
        const pokemonSpeciesUrl = `https://pokeapi.co/api/v2/pokemon-species/${pokeID}/`;
        ...
        const height =
            Math.round((pokemonRes.data.height * 0.328084 + 0.00001) * 100) /
            100;

        const weight =
            Math.round((pokemonRes.data.weight * 0.220462 + 0.00001) * 100) /
            100;
